...
$submit = (!empty($_POST['submit'])) ? ($_POST['submit']) : null;
  ...
  function createSelectCategory($array_cars, $cat_aliment){
  ...
  $arrayLength = count($array_cars[$cat_cars]);
  ...
  if(!empty($submit)){
    try{
      validate_aliment($car);
      //include_once "vues/manger.php";
      header("location:vues/manger.php?aliment=$car");
...
createSelectCategory($array_cars,$cat_aliment);
selectedCarModel($array_cars[$cat_aliment]);
